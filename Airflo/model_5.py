...
model_cnn = Sequential([
    Conv1D(filters=64, kernel_size=8, activation='relu', input_shape=(X_train_series.shape[1], X_train_series.shape[2]), kernel_regularizer=regularizer),
    BatchNormalization(),
    MaxPooling1D(pool_size=2),
    Conv1D(filters=128, kernel_size=8, activation='relu', kernel_regularizer=regularizer),
    BatchNormalization(),
    MaxPooling1D(pool_size=2),
    Flatten(),
...
monitor = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=100, verbose=1, mode='auto', restore_best_weights=True)
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=20, min_lr=0.0001, verbose=1)

cnn_history = model_cnn.fit(X_train_series, Y_train, callbacks=[monitor, reduce_lr],
