...
from keras.layers import Conv1D, Dense, Flatten, MaxPooling1D, LSTM, Dropout
...
# Define model with LayerNormalization
model_cnn_lstm = Sequential([
    LayerNormalization(input_shape=(X_train_series.shape[1], X_train_series.shape[2])),
...
monitor = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=50, verbose=1, mode='auto', restore_best_weights=True)
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=20, min_lr=0.00001, verbose=1)

# Fit model
cnn_lstm_history = model_cnn_lstm.fit(X_train_series, Y_train, callbacks=[monitor, reduce_lr],
                                      validation_data=(X_valid_series, Y_valid),
                                      epochs=epochs, batch_size=batch, verbose=2)

# Prediction
cnn_lstm_train_pred = model_cnn_lstm.predict(X_train_series)
cnn_lstm_valid_pred = model_cnn_lstm.predict(X_valid_series)
...
validation_loss = cnn_lstm_history.history['val_loss']
